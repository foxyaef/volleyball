"""V11을 보존하며 수비 준비와 진영 안전 공격을 추가 검증한 HybridRL V13 대회 봇."""
...
def decide(s):
    ...
    if incoming:
        ...
        ready_x = 124.0 if opponent_near_net and ball_on_opponent_side else 108.0
        dx = ready_x - self_x
...
ATTACK_WEIGHTS_LEFT = [
 ...
 0.14508898238795082,
 0.08740734549978504,
 -0.006203401706440239,
 -8.330808696011498,
 -0.14028161598326744,
 0.22532751722872155,
 0.001146178896665133,
 -0.08536346322799043,
 -0.0134130994552661,
 0.3734679733070835,
 0.21997505913033039,
 0.16980208254897788,
 0.0024352427197765966,
 1.1416805000474126,
 1.2830748690437492,
 -0.1453665093888168,
 0.06051398931571976,
 ...
 -0.2391846075736776,
 -0.07607876142947456,
 -0.056201057515327106,
 1.8799790607278015,
 -0.315586832944235,
 0.6375903284286122,
 0.046256401392626004,
 -0.07003580631380851,
 -0.16277005867421138,
 -0.36416670077715924,
 0.10096854834493667,
...
def _settings(weights=None):
    """학습된 앞 6개 값을 정렬·점프·타격 범위로 변환한다."""
    p = ATTACK_WEIGHTS if weights is None else weights
    return {
...
def _choose_shot(ball_x, ball_y, ball_vy, self_x, opponent_x, opponent_state, weights=None):
    ...
    for index, shot in enumerate(SHOT_TYPES):
        ...
        params = ATTACK_WEIGHTS if weights is None else weights
        row = params[start:start + SHOT_FEATURE_COUNT]
...
def basic_attack_candidate(s):
    ...
    attack_weights = ATTACK_WEIGHTS_LEFT if side == 'LEFT' else ATTACK_WEIGHTS
    settings = _settings(attack_weights)
